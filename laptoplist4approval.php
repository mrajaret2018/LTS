<?php
	...
	$sql = "select c.name, b.reason, b.committedrtndate, b.laptopid,b.requesterid,b.requestdate from master a, details b, users c where a.status = 'REQUESTED' and a.laptopid = b.laptopid and b.requesterid = c.empid";
    $rtnstr = '<form>';
	$rowid = 0;
    if ($link->real_query($sql) === TRUE) {
        ...
        while ($row = $result->fetch_row()) {
            $CHKBOXstuff = '<input type="checkbox" name="approvedorrejectedLP" value="'; 
        ...
        $rtnstr .= '<button type="button" onclick="Approve_Reject('."'Approve'".');" > Approve</button>';
        $rtnstr .= '<button type="button" onclick="Approve_Reject('."'Reject'".');" > REJECT</button>';
        $rtnstr .= '</form>';
    }
