<?php
   ...
   // $debug = new PHPDebug();
	...
	$sql = "update details set approverid = '$empid', approvedate = '$currentdate' where laptopid ='$laptopid' and requesterid = '$requesterid' and requestdate = '$requestdate' ";
    //$debug->debug("Variable sql: ", $sql);
     
	$rtnstr = "Just initialised";
    if ($link->real_query($sql) === TRUE) {
        $sql = "update master set status = 'APPROVED' where laptopid = '$laptopid'";
        if ($link->real_query($sql) === TRUE) {      
            $rtnstr = 'Your request is Approved';
        }
    }
